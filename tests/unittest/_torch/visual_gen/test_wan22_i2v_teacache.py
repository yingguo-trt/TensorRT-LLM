...
"""Tests for Wan 2.2 I2V TeaCache happy path.
...
Run:
    pytest tests/unittest/_torch/visual_gen/test_wan22_i2v_teacache.py -v -s
...
WAN22_I2V_HIGH_NOISE_COEFFICIENTS = [
    -5784.54975374,
    5449.50911966,
    -1811.16591783,
    256.27178429,
...
WAN22_I2V_LOW_NOISE_COEFFICIENTS = [
    2.39676752e03,
    -1.31110545e03,
    2.01331979e02,
    -8.29855975e00,
...
@pytest.fixture
def wan22_i2v_pipeline():
    if not os.path.exists(WAN22_I2V_A14B_PATH):
        pytest.skip(f"Checkpoint not found: {WAN22_I2V_A14B_PATH}")
    args = VisualGenArgs(
        model=WAN22_I2V_A14B_PATH,
        cache_config=TeaCacheConfig(
            teacache_thresh=0.15,
            coefficients=WAN22_I2V_HIGH_NOISE_COEFFICIENTS,
            coefficients_2=WAN22_I2V_LOW_NOISE_COEFFICIENTS,
        ),
    )
...
def _assert_dual_stage_teacache(pipeline, height: int, width: int) -> None:
    ...
    print(f"\n  ===== TeaCache — Wan 2.2 I2V-A14B dual-stage {height}x{width} =====")
...
class TestWan22I2V_TeaCache:
    ...
    def test_wan22_i2v_teacache_two_backends_initialized(self, wan22_i2v_pipeline):
        assert len(wan22_i2v_pipeline.cache_accelerator.backends) == 2
